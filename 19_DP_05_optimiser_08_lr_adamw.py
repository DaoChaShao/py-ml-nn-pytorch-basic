...
# -*- Coding: UTF-8 -*-
# @Time     :   2025/10/18 00:14
# @Author   :   Shawn
# @Version  :   Version 0.1.0
# @File     :   19_DP_05_optimiser_08_lr_adamw.py
# @Desc     :   
...
def train():
    with RandomSeed("Optimiser with Adam Learning Rate Scheduler"):
        device = "cpu"
        ...
        params_ADAM = X.clone().detach().requires_grad_(True)
        beta1: float = 0.9
        beta2: float = 0.999
        optimizer_ADAM = optim.Adam([params_ADAM], lr=ALPHA, betas=(beta1, beta2))
        ...
        params_ADAMW = X.clone().detach().requires_grad_(True)
        optimizer_ADAMW = optim.AdamW([params_ADAMW], lr=ALPHA, betas=(beta1, beta2))

        # Forward pass
        alphas, outs_ADAM = grad_descent(params_ADAM, optimizer_ADAM, EPOCHS, BATCHES)
        _, outs_SGD = grad_descent(params_SGD, optimizer_SGD, EPOCHS, BATCHES)
        _, outs_ADAMW = grad_descent(params_ADAMW, optimizer_ADAMW, EPOCHS, BATCHES)

    return alphas, outs_ADAM, outs_SGD, outs_ADAMW
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self._line_titles: list[str] = [
            "Learning Rate",
            "Adam Function Output",
            "SGD Function Output",
            "AdamW Function Output",
        ]
